...
事前シグナルとして使います：プラン別のしきい値（Max 5x → 0.96、Max 20x → 0.98、`claudeMaxTier` から
...
- **2 つのフラグファイルは意味が正反対——消し間違えに注意**：`<data_dir>/limiter-tripped` は**ラッチ本体**（消す＝作業再開、保護は有効なまま）、`<data_dir>/limiter-off` は**機能全体の無効化**（**作成するとラッチも解放されます**、消す＝保護が戻る）。
- **水位が同じしきい値を下回るとラッチは自動的に解除されます**——5 時間枠が切り替わって utilization がゼロに戻る瞬間が通常そのタイミングで、人の操作は不要です。
- **proxy が動いていなくても詰みません**：guard は 5 時間枠より古いラッチファイルを無視して通します。daemon のクラッシュ、マシンの再起動、proxy 経由をやめた場合でも、ラッチがツール呼び出しを永久に拒否することはありません。
- **ラッチ中に遅くなるのは仕様であり、ハングではありません。** limiter の目的は「自分では間に合わない一時停止を機械が代わりに行う」ことなので、`/loop` やスケジュールジョブは減速しますが、**長くても現在の 5 時間枠の終わりまで**で、人が戻らなくても回復します。より早く戻したい場合はラッチファイルを削除してください。
- **解除しきい値はトリガーしきい値と同一の値**で、ヒステリシス帯は設けません：5 時間枠は固定枠なので、枠内では水位は上がる一方で、切り替え時にゼロへ戻ります。しきい値付近で往復する条件がそもそもありません。
