...
（Max 5x → 0.90、Max 20x → 0.95，依 `claudeMaxTier` 自動偵測）時，proxy **建立閂鎖並持住所有 API
流量**，pacing-guard 同時擋下工具呼叫並印出原因。

- **opt-in 兩步**：先設好 `ANTHROPIC_BASE_URL` 導流（同上），再加 `RATE_LIMIT_PROXY_LIMITER=1`。不設＝行為完全不變。
- **兩個旗標檔語意相反，別刪錯**：`<data_dir>/limiter-tripped` 是**閂鎖本身**（刪掉＝恢復工作，保護仍在）；`<data_dir>/limiter-off` 是**停用整個功能**（刪掉＝保護回來）。
- **unattended 長跑會被閂到人回來——這是特性不是當機。** limiter 的用途正是「機器執行那個你來不及執行的暫停」，所以 `/loop`、排程 job 會停住等人刪閂鎖檔。
