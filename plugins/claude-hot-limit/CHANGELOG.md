...
- **feat（rate-state.jsonl size-based rotation，#17）**：production 實測推翻原「量不大→won't-fix」（48MB/9 天、#12 後 ~15MB/day）。`write_state_record` 在既有 flock 臨界區內做 size 檢查：live 檔 > `RATE_LIMIT_PROXY_ROTATE_MB`（**float MB**，預設 **64** ≈ 4 天；測試可設微 cap）→ rename 成 `rate-state-<YYYYmmdd-HHMMSS>.jsonl`（同秒碰撞加序號後綴）後開新檔續寫。**archive 全保留**——歷史 record 是校準語料（#23/#25 分析資料集），rotation 只讓 live 檔有界、不刪資料，prune 手動。臨界區內 rename + 每次寫入重新開檔（無持久 fd）= **零 record 遺失**。壞值紀律比照 DRAIN_CAP：非有限/parse 失敗 → 預設；**≤0 → 停用**（escape hatch）；rotation 失敗 fail-open（警告 + 照常寫入——寧可檔案續長，不可丟 record）。
- **feat（proxy.log spawn-time 輪替，#17）**：119MB/9 天、成長比 rate-state 快。`ensure` 在 spawn 前（daemon 未跑、無人持 fd 的唯一安全點）若 proxy.log > `RATE_LIMIT_PROXY_LOG_ROTATE_MB`（float MB，預設 **32**）→ `os.replace` 成 `proxy.log.1`（**只留一代**——log 無語料價值）。**誠實邊界**：mid-run 成長無界至下次重啟（stderr fd 固定；daemon 端 reopen 屬過度工程，部署節奏下可接受）。
