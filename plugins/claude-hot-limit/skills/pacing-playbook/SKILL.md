...
|------|------|
| 寬(`parallel` / `pipeline`,或多個 `agent()`) | **pin 便宜 model**:`agent(..., {model:'sonnet'})`(或 `haiku`)。省 token、少撞牆 |
| 窄(1-2 個 sequential agent) | 用預設(繼承 session,通常 opus)可負擔 |
...
改 model 的決定在你手上——同 `~/.claude/CLAUDE.md` 的 always-on doctrine)。
