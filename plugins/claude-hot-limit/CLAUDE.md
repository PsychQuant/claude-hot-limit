...
- **fan-out 寬度 → dispatch model 建議（1.12.0，#19）**：launch `Workflow` 時，`estimate_workflow_fanout()` 從 `tool_input` 粗估 fan-out 寬度（inline `script` 直接 parse；`scriptPath` bounded 讀檔；name/resume 估不到 → 跳過），**寬**（`parallel`/`pipeline` 或 ≥4 個 `agent()`）→ 注入 `systemMessage` 提醒「script 裡 pin `agent(..., {model:'sonnet'})`，別繼承 session 貴 model」。**只顯示不擋**（hook 改不了 model）、**靜態估**（dynamic/budget-scaled 估不到，明講「runtime 可能更寬」）、fail-open（估不到不注入）、同受 `_WORKFLOW_NUDGE` 開關、只 `Workflow`。**決定 model** 的機制在 `~/.claude/CLAUDE.md` 的 always-on doctrine（此 hook 是 runtime backstop）。實測 238 個真實 Workflow：inline 24% / scriptPath 66% / name 11%，故必讀兩種。
