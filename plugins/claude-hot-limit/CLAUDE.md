...
- **rotation 邊界（#17）**：`proxy.log` 只在 daemon spawn 時輪替——**mid-run 成長無界至下次
  重啟**（stderr fd 固定；部署節奏 = 每次 release 都 restart，可接受）。`launches.jsonl` /
  `trips-raw.jsonl` **明文 won't-fix**：KB 級（hook 頻率）+ 跨行程 flock 輪替風險 > 收益——
  原「量不大」判斷對這兩個檔案成立；`proxy-headers-debug.jsonl` 為 opt-in、既有「查完刪」紀律涵蓋。
