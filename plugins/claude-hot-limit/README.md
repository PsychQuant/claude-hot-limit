...
> 重啟 session。手動管理：`python3 proxy/proxy-launcher.py stop [--force]|restart|status`（`stop` 預設 graceful drain、部署重啟一律用 `restart`；#27）。
...
| `<data_dir>/limiter-off` | **停用旗標**：整個 limiter 不作用。**建立它會連帶刪除閂鎖檔**（停用＝釋放，不是凍結） | 讓 limiter 重新開始作用；閂鎖依**當下水位**重新判定，不會有殘留閂鎖復活 |

兩個檔名只差一個詞，但**刪錯的後果是相反的**：想恢復工作的人若誤刪 `limiter-off`，limiter
會立刻回來（水位仍高就重新閂住他）；想關掉保護的人若誤刪 `limiter-tripped`，保護其實還開著。
閂鎖檔本身的內容也會提醒這件事——它記錄觸發時間、當時 utilization、當時門檻、偵測到的方案別，
以及解除方式。

**停用即釋放（v1.22.0 起）**：三條停用路徑（`limiter-off`、`RATE_LIMIT_PROXY_LIMITER` 未設、
hold cap ≤ 0）在生效時都會**刪除閂鎖檔**。早期版本只是跳過整段邏輯，於是停用把閂鎖凍結在磁碟上
——proxy 不再持住流量，但 pacing-guard 仍依閂鎖檔存在與否 deny 每個工具呼叫，而唯一會清它的
自動解除分支永遠到不了。結果是最顯眼的復原動作反而讓中斷變成永久。**disable 必須意味著
release，不是 freeze。**
...
2. **手動**：刪除 `<data_dir>/limiter-tripped`，立即生效、不需重啟 daemon。水位確實仍在門檻之上
   時，這是唯一能強制放行的手段。
3. **停用**：建立 `<data_dir>/limiter-off`（或移除 opt-in 環境變數並重啟）——連帶釋放閂鎖。

**proxy 不在場的兜底**：以上三條都需要 proxy 還在跑。若 daemon 崩潰、機器重開、或流量不再經過
proxy，閂鎖檔就沒有任何程序會清它。因此 **pacing-guard 會忽略年齡超過一個 5 小時窗的閂鎖並放行**
——閂鎖比它保護的那個窗還老，就代表沒有 proxy 在釋放它。年齡取閂鎖檔內的機器可讀觸發時間，
解析不出則退回檔案 mtime（涵蓋舊版寫的檔與被截斷的殘檔）。這是 guard 唯一自行判定的量，判的是
**時間**不是門檻，「guard 不擁有門檻」的紀律仍然成立。
